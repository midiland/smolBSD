...
$ make MOUNTRO=y rescue # create a rescue image (base-amd64.img)
...
$ sudo ln -sf /var/run/nitro.sock mnt/etc/nitro/nitro.sock # link to absolute path, it will work when running from that fs
$ umount ./mnt
$ ./startnb.sh -k kernels/netbsd-MICROVM -i images/frankenbase.img -a "-v" # TRY it another time => PANIC !
$ sudo mount -o loop images/frankenbase.img mnt # mount again
$ sudo mv mnt/etc/nitro/nitro.sock mnt/usr/pkg/etc/nitro.sock # fix the link to the proper location
$ sudo umount mnt
